  ...
  return (
    <div style={{ background: 'var(--bg2)', borderRadius: 14, overflow: 'hidden', border: '1px solid var(--border)', marginBottom: 12, opacity: task.done ? 0.5 : 1 }}>
      ...
      <div style={{ padding: '14px 14px 12px' }}>
        ...
        {/* Title */}
        <p onClick={() => onEdit(taskId, task)} style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, lineHeight: 1.35, color: 'var(--text)', textDecoration: task.done ? 'line-through' : 'none', marginBottom: hasDesc ? 8 : 0, cursor: 'pointer' }}>{task.text}</p>

        {/* Description preview */}
        {hasDesc && !expanded && (
          <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 14, color: 'var(--text3)', lineHeight: 1.5, marginBottom: 8, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {task.description}
          </p>
        )}
        ...
        {subtasks.length > 0 && (
          <div style={{ marginBottom: 8 }}>
            <div style={{ height: 3, background: 'var(--border)', borderRadius: 2, marginBottom: 6, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${(doneCount / subtasks.length) * 100}%`, background: accent, transition: 'width 0.3s' }} />
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
          {dl && !task.done && (
            <span style={{ fontSize: 10, letterSpacing: 1, color: dl.overdue ? '#7A3525' : dl.soon ? '#5C3D10' : 'var(--text3)', background: dl.overdue ? '#F5EAE7' : dl.soon ? '#F2EBE0' : 'var(--bg3)', padding: '3px 8px', borderRadius: 20 }}>
              {dl.overdue ? `просрочено ${Math.abs(dl.diff)}д` : dl.diff === 0 ? 'сегодня' : dl.fmt}
            </span>
          )}
          {task.addedBy && task.addedBy !== 'Олжас' && <span style={{ fontSize: 10, color: 'var(--text3)' }}>+ {task.addedBy}</span>}
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 10, alignItems: 'center' }}>
            <button onClick={() => setExpanded(!expanded)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, color: expanded ? accent : 'var(--text3)', letterSpacing: 0.5 }}>
              {expanded ? '▲' : (hasDesc || comments.length > 0 || subtasks.length > 0) ? `▼${comments.length > 0 ? ` ${comments.length}💬` : ''}` : '▼'}
            </button>
            <button onClick={toggleDone} style={{ width: 22, height: 22, borderRadius: '50%', border: `1.5px solid ${task.done ? accent : 'var(--border)'}`, background: task.done ? accent : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {task.done && <span style={{ color: 'var(--bg)', fontSize: 10 }}>✓</span>}
            </button>
          </div>
        </div>
      </div>
      ...
      {expanded && (
        <div style={{ borderTop: '1px solid var(--border)', background: 'var(--bg)' }}>
          ...
          {/* Description / ТЗ */}
          {hasDesc && (
            <div style={{ padding: '16px 14px', borderBottom: subtasks.length > 0 || comments.length > 0 ? '1px solid var(--border)' : 'none' }}>
              <p style={{ fontSize: 9, letterSpacing: 2, color: accent, textTransform: 'uppercase', marginBottom: 8 }}>Описание / ТЗ</p>
              <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 16, color: 'var(--text2)', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{task.description}</p>
            </div>
          )}

          {/* Subtasks */}
          {subtasks.length > 0 && (
            <div style={{ padding: '12px 14px', borderBottom: comments.length > 0 ? '1px solid var(--border)' : 'none' }}>
              <p style={{ fontSize: 9, letterSpacing: 2, color: accent, textTransform: 'uppercase', marginBottom: 8 }}>Подзадачи</p>
              {subtasks.map(([sid, sub]) => (
          ...
          <div style={{ padding: '12px 14px' }}>
            {comments.length > 0 && (
              <div style={{ marginBottom: 10 }}>
                <p style={{ fontSize: 9, letterSpacing: 2, color: accent, textTransform: 'uppercase', marginBottom: 8 }}>Комментарии</p>
                {comments.sort((a,b) => a[1].ts - b[1].ts).map(([cid, c]) => (
                  <div key={cid} style={{ marginBottom: 8, paddingLeft: 8, borderLeft: `2px solid ${accent}44` }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <input value={comment} onChange={e => setComment(e.target.value)} onKeyDown={e => e.key === 'Enter' && addComment()} placeholder="комментарий…"
                style={{ flex: 1, background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 10px', fontSize: 12, color: 'var(--text)', outline: 'none' }} />
...
function TaskModal({ user, categories, editTask, editId, onClose }) {
  const isEdit = !!editId
  const [form, setForm] = useState(editTask || { text: '', category: categories[0], priority: 'medium', deadline: '', description: '' })
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,16,12,0.6)', zIndex: 200, display: 'flex', alignItems: 'flex-end' }} onClick={onClose}>
      <div style={{ width: '100%', background: 'var(--bg)', borderRadius: '20px 20px 0 0', padding: '20px', maxHeight: '92dvh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        ...
        <Label>Задача</Label>
        <textarea value={form.text} onChange={e => setForm(f => ({ ...f, text: e.target.value }))} rows={2} placeholder="Что нужно сделать?" style={inputStyle} />

        <Label>Описание / ТЗ</Label>
        <textarea value={form.description || ''} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={5}
          placeholder="Подробное описание, техническое задание, детали для команды…"
          style={{ ...inputStyle, fontFamily: "'Cormorant Garamond', serif", fontSize: 16, lineHeight: 1.6 }} />
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          <input value={subInput} onChange={e => setSubInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addSub())} placeholder="добавить подзадачу"
        ...
        <button onClick={save} disabled={saving} style={{ width: '100%', padding: '16px', background: 'var(--text)', color: 'var(--bg)', border: 'none', borderRadius: 10, cursor: 'pointer', fontFamily: "'Cormorant', serif", fontStyle: 'italic', fontSize: 22, marginBottom: 10, opacity: saving ? 0.6 : 1 }}>
...
export default function Tasks({ user, tasks, categories }) {
  const [activeCategory, setActive] = useState('all')
  const [modal, setModal] = useState(false)
  const [editData, setEditData] = useState({ id: null, task: null })
  const userCats = getUserCategories(user, categories)
  ...
  const filtered = Object.entries(tasks || {}).filter(([, t]) => {
    ...
    if (!userCats.includes(t.category)) return false
  ...
  return (
    <div style={{ paddingBottom: 90 }}>
      <div style={{ display: 'flex', gap: 0, overflowX: 'auto', padding: '0 16px', marginBottom: 20, borderBottom: '1px solid var(--border)', scrollbarWidth: 'none' }}>
        {[['all', 'Все'], ...userCats.map(c => [c, c])].map(([val, label]) => {
      ...
      {modal && <TaskModal user={user} categories={userCats} editTask={editData.task} editId={editData.id} onClose={closeModal} />}
    </div>
  )
}
